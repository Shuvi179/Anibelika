...
public interface BookRatingVoteRepository extends JpaRepository<BookRatingVote, Long> {
    ...
    @Query(value = "select brv from BookRatingVote brv join brv.bookRating br where brv.user.id = ?2 and br.book.id = ?1")
    BookRatingVote findByBookAndUserId(Long bookId, Long userId);
}
